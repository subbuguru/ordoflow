...
export function TodosProvider({
  ...
  const addTodo = async (input: string, description: string, priority: 'p1' | 'p2' | 'p3' | 'p4') => {
    ...
    await db.runAsync(`INSERT INTO todos (text, description, completed, date, priority, orderIndex) VALUES (?, ?, ?, ?, ?, ?);`, [input, description, 0, date, priority, newIndex]);
    await loadTodos();
  };
  const updateTodo = async (todo: Todo) => {
    await db.runAsync(`UPDATE todos SET text = ?, description = ?, priority = ? WHERE id = ?`, [todo.text, todo.description ?? '', todo.priority, Number(todo.id)]);
    await loadTodos();
  };
  const updateTodoOrder = async (reorderedTodos: Todo[]) => {
    await db.withTransactionAsync(async () => {
      for (let i = 0; i < reorderedTodos.length; i++) {
        await db.runAsync(`UPDATE todos SET orderIndex = ? WHERE id = ?`, [i, Number(reorderedTodos[i].id)]);
      }
    });
    await loadTodos();
  };
